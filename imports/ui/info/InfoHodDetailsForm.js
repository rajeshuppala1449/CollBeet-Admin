...
class HodDetailsForm extends Component {
  state = {
    open: false,
    deptAnchorEl: null,
    menuDept: "",
  ...
  handleSubmit = event => {
    ...
    const { hodContact, hodName, menuDeptCode, menuDept } = this.state;
    Meteor.call(
      "info.addhoddetail",
      hodContact,
      hodName,
  ...
  addButton() {
    const { hod_data } = this.props;
    const { menuDeptCode, hodContact, hodName } = this.state;

    const hod_deptId = hod_data
      .filter(function(d) {
      ...
      <div className={this.props.classes.root}>
        {hod_data.length != 0 ? (
          <div>
            ...
            {hod_data.map(({ _id, hodName, hodContact, department }) => (
              <div key={_id}>
        ...
        <TextField
          ...
          onChange={this.changeResponse("hodName")}
        />

        <TextField
          className={this.props.classes.textfield}
          label="HOD Email/Phone Number"
          variant="outlined"
          fullWidth
          value={this.state.hodContact}
          onChange={this.changeResponse("hodContact")}
        />
...
export default compose(
  ...
  withTracker(() => {
    ...
    return {
      hod_data: Info.find({ hoddetail: { $eq: true } }).fetch()
    };
  })
)(HodDetailsForm);
